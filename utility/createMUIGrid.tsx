...
const employee_columns = () => {
	const cols: GridColDef[] = [
		...
		},
		{
			field: "badgeNo",
			headerName: "Badge #",
			description: "The officer's BPD badge number",
			type: "string",
			width: 110,
		},
		{
			field: "postId",
			headerName: "POST ID #",
			description: "Statewide POST (Peace Officer Standards & Training) certification ID",
			type: "string",
			width: 130,
		},
		{
		...
		},
		{
			...
			description: "The officer's most recent assignment (unit / district)",
			type: "string",
			width: 200,
		},
		{
			field: "startDate",
			headerName: "Start Date",
			description: "Academy graduation date (earliest known), used as a proxy for BPD start date where available",
			type: "string",
			width: 130,
			valueFormatter: (params) => {
				if (!params.value) return "—";
				const d = new Date(String(params.value).slice(0, 10) + "T00:00:00Z");
				return isNaN(d.getTime()) ? "—" : d.toLocaleDateString(undefined, { timeZone: "UTC", year: "numeric", month: "short", day: "numeric" });
			},
		},
		{
			field: "residenceZip",
		...
		},
		{
			field: "numOfFio",
			headerName: "FIOs",
			description: "Number of Field Interrogation & Observation reports linked to the officer",
			type: "number",
			align: "center",
			width: 100,
		},
		{
			...
			description: "Number of incident reports linked to the officer (2020–2024)",
			type: "number",
			align: "center",
			width: 110,
		},
		{
			field: "numOfMvc",
			headerName: "Traffic Citations (MVC)",
			description: "Number of motor-vehicle citations (traffic stops) linked to the officer",
			type: "number",
			align: "center",
			width: 190,
		},
		...
		{
			field: "courtOtHours",
			headerName: "Court Overtime (hrs)",
			description: "Total court-overtime hours worked (source reports hours, not dollars)",
			type: "number",
			width: 170,
			valueFormatter: (params) => (params.value == null ? "—" : `${Number(params.value).toLocaleString()} hrs`),
		},
		{
			field: "salPlan",
