...
export const V2_OFFICER_COURT_OVERTIME = (bpiId: string) => gql`
	query MyQuery {
		${v2_court_overtime_alias_name}(condition: {bpiId: "${bpiId}"}, orderBy: OT_DATE_DESC) {
			nodes {
				bpiId
				otDate
				otCode
...
export const V2_OFFICER_ARROYO = (bpiId: string) => gql`
	query MyQuery {
		${v2_officer_arroyo_alias_name}(condition: {bpiId: "${bpiId}"}) {
			nodes {
				bpiId
				servingSince
				lastSeenYear
