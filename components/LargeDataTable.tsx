...
export const CustomToolbar = (table_name) => {
...
export default function LargeDataTable({ cols, table, table_name, height, pageSize, pageSizeOptions, rowCount }: { cols: GridColDef[]; table: any[]; table_name: string; height: string; pageSize: number; pageSizeOptions: number[]; rowCount: number | undefined }) {
	...
	return (
		<DataGrid
			...
			columns={cols}
			rows={table}
			// onFilterModelChange={(e) => console.log("filtering", e.items[0].field, e.items[0].value)}
			density={"compact"}
			slots={{
